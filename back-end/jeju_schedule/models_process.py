...
from jeju_data.models import Plane, Restaurant, Shop, Olle, Tourism, Activity, Accommodation, PlaneCategory
...
class JejuProcess:
    ...
    def activty_values(self, dic, i, except_restaurant_id, except_tourism_id, except_shop_id):
        ...
        a = Restaurant.objects.raw(
            ...
            'AS distance FROM tripn_mariadb.restaurant where id not in %s ORDER BY distance', [i['lat'], i['log'], i['lat'], at])[:2]
        # a = Restaurant.objects.raw(
        #     'SELECT *, (6371*acos(cos(radians(%s))*cos(radians(lat))*cos(radians(log)'
        #     '-radians(%s))+sin(radians(%s))*sin(radians(lat))))'
        #     'AS distance FROM tripn_mariadb.restaurant where id not in %s ORDER BY distance' % [i['lat'], i['log'], i['lat'], at])[:2]
        ...
        b = Tourism.objects.raw(
            "SELECT *, (6371*acos(cos(radians(%s))*cos(radians(lat))*cos(radians(log)"
            "-radians(%s))+sin(radians(%s))*sin(radians(lat))))"
            "AS distance FROM tripn_mariadb.tourism where id not in %s and tour_category_id not in (8, 12) ORDER BY distance", [i['lat'], i['log'], i['lat'], bt])[:2]
        b = TourismSerializer(b, many=True).data
        [except_tourism_id.append(list(i.values())[0]) for i in b]
        c = Activity.objects.raw(
            "SELECT *, (6371*acos(cos(radians(%s))*cos(radians(lat))*cos(radians(log)"
            "-radians(%s))+sin(radians(%s))*sin(radians(lat))))"
            "AS distance FROM tripn_mariadb.activity ORDER BY distance", [i['lat'], i['log'], i['lat']])[:1]
        ...
        d = Shop.objects.raw(
            ...
            '+sin(radians(%s))*sin(radians(lat)))) AS distance FROM tripn_mariadb.shop where id not in %s HAVING distance < 4 ORDER BY distance',
    ...
    def olle_values(self, dic, i, except_restaurant_id, except_tourism_id, except_shop_id):
        ...
        a = Restaurant.objects.raw(
            'SELECT *, (6371*acos(cos(radians(%s))*cos(radians(lat))*cos(radians(log)'
            '-radians(%s))+sin(radians(%s))*sin(radians(lat))))'
            'AS distance FROM tripn_mariadb.restaurant where id not in %s ORDER BY distance',
        ...
        b = Tourism.objects.raw(
            "SELECT *, (6371*acos(cos(radians(%s))*cos(radians(lat))*cos(radians(log)"
            "-radians(%s))+sin(radians(%s))*sin(radians(lat))))"
            "AS distance FROM tripn_mariadb.tourism where id not in %s and tour_category_id not in (8, 12) ORDER BY distance",
        ...
        c = Olle.objects.raw(
            "SELECT *, (6371*acos(cos(radians(%s))*cos(radians(lat))*cos(radians(log)"
            "-radians(%s))+sin(radians(%s))*sin(radians(lat))))"
            "AS distance FROM tripn_mariadb.olle ORDER BY distance", [i['lat'], i['log'], i['lat']])[:1]
    ...
    def oleum_values(self, dic, i, except_restaurant_id, except_tourism_id, except_shop_id):
        ...
        c = Tourism.objects.raw(
            "SELECT *, (6371*acos(cos(radians(%s))*cos(radians(lat))*cos(radians(log)"
            "-radians(%s))+sin(radians(%s))*sin(radians(lat))))"
            "AS distance FROM tripn_mariadb.tourism ORDER BY distance",
            [i['lat'], i['log'], i['lat']])[:1]
